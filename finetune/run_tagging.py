...
@dataclass
class DataCollatorForTokenMultiClassification(DataCollatorForTokenClassification):

    def __init__(self, *args, label_names=("label",), **kwargs):
    ...
    def torch_call(self, features):
        ...
        label_names = [label_name for label_name in self.label_names if label_name in features[0].keys()]
        labels = {label_name: [feature[label_name] for feature in features] for label_name in label_names if
                  label_name in features[0].keys()} or None

        no_labels_features = [{k: v for k, v in feature.items() if k not in self.label_names} for feature in features]
        ...
        for label_name in label_names:
            if padding_side == "right":
                batch[label_name] = [
                    to_list(label) + [self.label_pad_token_id] * (sequence_length - len(label)) for label in
                    labels[label_name]
                ]
            else:
                batch[label_name] = [
                    [self.label_pad_token_id] * (sequence_length - len(label)) + to_list(label) for label in
                    labels[label_name]
                ]
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        if data_args.remove_splits is not None:
            for
        split in data_args.remove_splits.split(","):
    ...
    for label_column_name in label_column_names:
        ...
        if label_column_name in model.config.label2id and model.config.label2id[label_column_name] != PretrainedConfig(
            ...
            if sorted(model.config.label2id[label_column_name].keys()) == sorted(label_list[label_column_name]):
                ...
                if labels_are_int:
                    label_to_id[label_column_name] = {i: int(model.config.label2id[l]) for i, l in
                                                      enumerate(label_list[label_column_name])}
                    label_list[label_column_name] = [model.config.id2label[i] for i in
                                                     range(num_labels[label_column_name])]
                else:
                    label_list[label_column_name] = [model.config.id2label[i] for i in
                                                     range(num_labels[label_column_name])]
                    label_to_id[label_column_name] = {l: i for i, l in enumerate(label_list[label_column_name])}
        ...
        for label_index, label_column_name in enumerate(label_column_names):
            ...
            for i, label in enumerate(examples[label_column_name]):
                ...
                for word_idx in word_ids:
                    ...
                    word_label = word_label if word_label is None or labels_are_int[label_column_name] else \
                    word_label.split(":")[0]
    ...
    # Data collator
    data_collator = DataCollatorForTokenMultiClassification(tokenizer, label_names="labels", pad_to_multiple_of=8 if training_args.fp16 else None)
    ...
    def compute_metrics(p):
        ...
        assert len(label_column_names) == len(all_predictions) == len(
            all_labels), "Number of overall predictions and labels should be the same as the number of labels"
